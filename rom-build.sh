#!/usr/bin/env bash
# The Syberia-Project build helper script
...
function showHelpAndExit {
        ...
        echo -e "${CLR_BLD_BLU}  -v, --variant         Syberia build type - Can be alpha, beta or release${CLR_RST}"
...
while true; do
    case "$1" in
        ...
        -v|--variant|v|variant) SYBERIA_BUILD_TYPE="$2"; shift;;
        -t|--build-type|t|build-type) DEVICE_BUILD_TYPE="$2"; shift;;
...
if [ ! -d "$DIR_ROOT/vendor/syberia" ]; then
        echo -e "${CLR_BLD_RED}error: insane root directory ($DIR_ROOT)${CLR_RST}"
        exit 1
fi

# Setup Syberia build type if specified
if [ $SYBERIA_BUILD_TYPE ]; then
    SYBERIA_BUILD_TYPE=`echo $SYBERIA_BUILD_TYPE |  tr "[:upper:]" "[:lower:]"`
    if [ "${SYBERIA_BUILD_TYPE}" = "release" ]; then
        export SYBERIA_BUILD_TYPE=RELEASE
    elif [ "${SYBERIA_BUILD_TYPE}" = "beta" ]; then
        export SYBERIA_BUILD_TYPE=BETA
    elif [ "${SYBERIA_BUILD_TYPE}" = "alpha" ]; then
        export SYBERIA_BUILD_TYPE=ALPHA
    else
	export SYBERIA_BUILD_TYPE=$SYBERIA_BUILD_TYPE
    fi
fi
...
echo -e "${CLR_BLD_GRN}Building Syberia Project for $DEVICE${CLR_RST}"
...
echo -e ""
lunch "syberia_$DEVICE-$DEVICE_BUILD_TYPE"
SYBERIA_VERSION="$(get_build_var SYBERIA_VERSION)"
TARGET_BACON_NAME="$(get_build_var TARGET_BACON_NAME)"
checkExit
echo -e ""
...
# Build signed rom package if specified
    ...
    sign_target_files_apks -o -d $KEY_MAPPINGS \
        "$OUT"/obj/PACKAGING/target_files_intermediates/syberia_$DEVICE-target_files-$FILE_NAME_TAG.zip \
        syberia-$DEVICE-$SYBERIA_VERSION-signed-target_files-$FILE_NAME_TAG.zip
...
# Build rom package
    ...
    mv -f $OUT/syberia_$DEVICE-ota*zip $OUT/$TARGET_BACON_NAME.zip
    echo "Package Complete: $OUT/$TARGET_BACON_NAME.zip"
fi
echo -e ""
